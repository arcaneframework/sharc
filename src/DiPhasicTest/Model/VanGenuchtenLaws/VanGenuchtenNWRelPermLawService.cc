...
//-----------------------------------------------------------------------------
// -*- C++ -*-
/* Author : encheryg at Fri Feb  3 09:46:38 2017
...
#include "VanGenuchtenNWRelPermLaw_axl.h"

/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/

class VanGenuchtenNWRelPermLawService 
  : public ArcaneVanGenuchtenNWRelPermLawObject
{
public:
  
  VanGenuchtenNWRelPermLawService(const Arcane::ServiceBuildInfo & sbi) 
    : ArcaneVanGenuchtenNWRelPermLawObject(sbi) {}
  
  ~VanGenuchtenNWRelPermLawService() {}
    ...
    void _compute(const Arcane::Real value, Arcane::Real& result) const {
      ...
      const Arcane::Real truncS = (value2 < m_swr) ? 0. : ((value2 > 1 - m_snr) ? 1. : (value2 - m_swr)/(1 - m_snr - m_swr)) ;
       ...
       if(truncS <= m_selow) {
         ...
         const Arcane::Real tmp = pow(1. - pow(truncS, 1. / m_mvg), m_mvg) ;
         result = cbrt(1. - truncS) * tmp * tmp ;
       }
    }
    ...
    void compute(const Arcane::Real value, Arcane::Real& result, Arcane::Real& deriv) const {
      ...
      if(value2 < m_swr || value2 > 1. - m_snr) {
        ...
        if(normS <= m_selow) {
          ...
          _compute(value, result) ;
          Arcane::Real tmp = pow(1. - pow(normS, 1. / m_mvg), m_mvg) ;
          Arcane::Real tmp2 = cbrt(1. - normS) ;
          deriv = (-1. / (3 * tmp2 * tmp2) * tmp * tmp -
                      tmp2 * 2. * pow(1. - pow(normS, 1. / m_mvg), 2 * m_mvg - 1.) * 
...
initialize(const Law::ScalarRealProperty& p, Law::FunctionManager& law_register)
{
  ...
  m_algo.m_cslow.reset(new CubicSpline(0.0, selow, 1., valueAtSelow, 0., derivAtSelow[0])) ;

  Arcane::Real valueAtSehigh(0.) ;
  Arcane::RealUniqueArray derivAtSehigh(1, 0.) ;
  m_algo.eval(sehigh, valueAtSehigh, derivAtSehigh[0]) ;
  m_algo.m_cshigh.reset(new CubicSpline(sehigh, 1.0, valueAtSehigh, 0., derivAtSehigh[0], 0.)) ;
...
ARCANE_REGISTER_SERVICE_VANGENUCHTENNWRELPERMLAW(VanGenuchtenNWRelPermLaw,VanGenuchtenNWRelPermLawService);
