...
{
  _checkInit();
  AlienDoFLinearSystemImpl* ptr = dynamic_cast<AlienDoFLinearSystemImpl*>(m_p) ;
  if(ptr)
    ptr->startSystemAssembly() ;
}

void DoFLinearSystem::endSystemAssembly()
{
  _checkInit();
   AlienDoFLinearSystemImpl* ptr = dynamic_cast<AlienDoFLinearSystemImpl*>(m_p) ;
   if(ptr)
     ptr->endSystemAssembly() ;
}

AlienBSRFormat* DoFLinearSystem::
alienBsr(ITraceMng* trace_mng,
         RunQueue& queue,
         FemDoFsOnNodes& dofs_on_nodes)
{
  ...
  AlienDoFLinearSystemImpl* ptr = dynamic_cast<AlienDoFLinearSystemImpl*>(m_p) ;
  if(ptr)
    return ptr->bsr(trace_mng,queue,dofs_on_nodes) ;
  else
    return nullptr;
}
