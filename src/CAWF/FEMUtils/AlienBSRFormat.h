 ...
 *
 * The `AlienBSRMatrix` class stores sparse matrices in the Block Sparse Row (BSR)
 ...
 */
class AlienBSRMatrix
: public TraceAccessor
{
  friend class AlienBSRFormat;
 ...
 * It uses a `AlienBSRMatrix` under the hood for representation and operations.
 *
...
: public TraceAccessor
{
 ...
 private:
  ...
  UniqueArray<Arccore::Integer> m_allUIndex;

  Alien::ArcaneTools::Space m_space;
  ...
  std::unique_ptr<Alien::ProfiledMatrixBuilder> m_builder;


  AlienBSRMatrix m_bsr_matrix;
...
template <typename ProfileT>
void AlienBSRFormat::
computeProfile(const std::function<void(ProfileT&, ConstArrayView<Integer>)>& compute_matrix_profile)
{
  ProfileT profiler(m_matrixA);
  compute_matrix_profile(profiler,m_allUIndex.constView()) ;
}
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/

template <class Function> inline void AlienBSRFormat::
assembleBilinearOrderedPerBlock(Function compute_element_matrix)
{
 ...
 *
 */
template <class Function> void AlienBSRFormat::
assembleBilinearAtomic(Function compute_element_matrix)
{
  info() << "AlienBSRFormat(assembleBilinearAtomic): Integrating over elements...";
...
assembleBilinearAtomicFree(Function compute_element_vectors)
{
  info() << "AlienBSRFormat(assembleBilinearAtomicFree): Integrating over nodes...";
