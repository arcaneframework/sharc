...
ARCCORE_HOST_DEVICE RealMatrix<6, 6> computeElementMatrixTria3BaseSYCL(Real3 dxu, Real3 dyu, Real area, Real lambda, Real mu)
{
                             ...
                             const Accelerator::VariableCellRealInView& mu)
{
  ...
  return computeElementMatrixTria3BaseSYCL(dxu, dyu, area, lambda[cell_lid], mu[cell_lid]);
}
...
computeElementVectorTria3SYCL(CellLocalId cell_lid,
                             const IndexedCellNodeConnectivityView& cn_cv,
                             const Accelerator::VariableNodeReal3InView& in_node_coord,
                             const Accelerator::VariableCellRealInView& lambda,
                             const Accelerator::VariableCellRealInView& mu,
  ...
  // ∫∫ (λ+2μ)(∂𝑢𝑥/∂𝑥 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑦/∂𝑦 ∂𝑣𝑦/∂𝑦)
  RealVector<6> normal_strain_energy_x = (lambda[cell_lid] + 2 * mu[cell_lid]) * ((dxUx(node_lid*2) * dxUx) + (dyUy(node_lid*2) * dyUy)) * area;
  RealVector<6> normal_strain_energy_y = (lambda[cell_lid] + 2 * mu[cell_lid]) * ((dxUx(node_lid*2+1) * dxUx) + (dyUy(node_lid*2+1) * dyUy)) * area;

  // ∫∫ λ(∂𝑢𝑦/∂𝑦 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑥/∂𝑥 ∂𝑣𝑦/∂𝑦)
  RealVector<6> compressibility_effect_x = (lambda[cell_lid]) * ((dyUy(node_lid*2) * dxUx) + (dxUx(node_lid*2) * dyUy)) * area;
  RealVector<6> compressibility_effect_y = (lambda[cell_lid]) * ((dyUy(node_lid*2+1) * dxUx) + (dxUx(node_lid*2+1) * dyUy)) * area;

  // ∫∫ μ(∂𝑢𝑦/∂𝑥 + ∂𝑢𝑥/∂𝑦)(∂𝑣𝑥/∂𝑦 + ∂𝑣𝑦/∂𝑥)
  RealVector<6> shear_energy_x = (mu[cell_lid]) * ((dxUy(node_lid*2) + dyUx(node_lid*2)) * (dyUx + dxUy)) * area;
  RealVector<6> shear_energy_y = (mu[cell_lid]) * ((dxUy(node_lid*2+1) + dyUx(node_lid*2+1)) * (dyUx + dxUy)) * area;
...
ARCCORE_HOST_DEVICE RealMatrix<12, 12> computeElementMatrixTetra4BaseSYCL(Real4 dxu, Real4 dyu, Real4 dzu, Real volume, Real lambda, Real mu){
...
computeElementMatrixTetra4SYCL(CellLocalId cell_lid,
  ...
  return computeElementMatrixTetra4BaseSYCL(dxu, dyu, dzu, volume, lambda[cell_lid], mu[cell_lid]);
}
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
...
computeElementVectorTetra4SYCL(CellLocalId cell_lid,
 ...
 //            ∂𝑢𝑦/∂𝑦 ∂𝑣𝑦/∂𝑦 +
 //            ∂𝑢𝑧/∂𝑧 ∂𝑣𝑧/∂𝑧 )
 RealVector<12> normal_strain_energy_x = (lambda[cell_lid] + 2 * mu[cell_lid]) * ( (dxUx(node_lid*3) * dxUx) +
                                                                   (dyUy(node_lid*3) * dyUy) +
                                                                   (dzUz(node_lid*3) * dzUz) ) * volume;

 RealVector<12> normal_strain_energy_y = (lambda[cell_lid] + 2 * mu[cell_lid]) * ( (dxUx(node_lid*3+1) * dxUx) +
                                                                   (dyUy(node_lid*3+1) * dyUy) +
                                                                   (dzUz(node_lid*3+1) * dzUz) ) * volume;

 RealVector<12> normal_strain_energy_z = (lambda[cell_lid] + 2 * mu[cell_lid]) * ( (dxUx(node_lid*3+2) * dxUx) +
  ...
  //     + ∂𝑢𝑧/∂𝑧 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑥/∂𝑥 ∂𝑣𝑧/∂𝑧
  //     + ∂𝑢𝑦/∂𝑦 ∂𝑣𝑧/∂𝑧 + ∂𝑢𝑧/∂𝑧 ∂𝑣𝑦/∂𝑦)
  RealVector<12> compressibility_effect_x = (lambda[cell_lid]) * ((dyUy(node_lid*3) * dxUx) + (dxUx(node_lid*3) * dyUy) +
                                                            (dzUz(node_lid*3) * dxUx) + (dxUx(node_lid*3) * dzUz) +
                                                            (dyUy(node_lid*3) * dzUz) + (dzUz(node_lid*3) * dyUy) ) * volume;

  RealVector<12> compressibility_effect_y = (lambda[cell_lid]) * ((dyUy(node_lid*3+1) * dxUx) + (dxUx(node_lid*3+1) * dyUy) +
                                                            (dzUz(node_lid*3+1) * dxUx) + (dxUx(node_lid*3+1) * dzUz) +
                                                            (dyUy(node_lid*3+1) * dzUz) + (dzUz(node_lid*3+1) * dyUy) ) * volume;

  RealVector<12> compressibility_effect_z = (lambda[cell_lid]) * ((dyUy(node_lid*3+2) * dxUx) + (dxUx(node_lid*3+2) * dyUy) +
  ...
  //     μ(∂𝑢𝑧/∂𝑦 + ∂𝑢𝑦/∂𝑧)(∂𝑣𝑦/∂𝑧 + ∂𝑣𝑧/∂𝑦) +
  //     μ(∂𝑢𝑥/∂𝑧 + ∂𝑢𝑧/∂𝑥)(∂𝑣𝑧/∂𝑥 + ∂𝑣𝑥/∂𝑧)
  RealVector<12> shear_energy_x = (mu[cell_lid]) * ( ((dxUy(node_lid*3) + dyUx(node_lid*3)) * (dyUx + dxUy)) +
                                               ((dzUy(node_lid*3) + dyUz(node_lid*3)) * (dyUz + dzUy)) +
                                               ((dxUz(node_lid*3) + dzUx(node_lid*3)) * (dzUx + dxUz)) ) * volume;

  RealVector<12> shear_energy_y = (mu[cell_lid]) * ( ((dxUy(node_lid*3+1) + dyUx(node_lid*3+1)) * (dyUx + dxUy)) +
                                               ((dzUy(node_lid*3+1) + dyUz(node_lid*3+1)) * (dyUz + dzUy)) +
                                               ((dxUz(node_lid*3+1) + dzUx(node_lid*3+1)) * (dzUx + dxUz)) ) * volume;



  RealVector<12> shear_energy_z = (mu[cell_lid]) * ( ((dxUy(node_lid*3+2) + dyUx(node_lid*3+2)) * (dyUx + dxUy)) +
