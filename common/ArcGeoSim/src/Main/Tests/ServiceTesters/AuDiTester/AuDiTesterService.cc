...
test()
{
  ...
  using ADoubleSparseWrap = audi::scalar_wrapper<Arcane::Real>;
  using ADoubleSparse = audi::sparse_expression<Arcane::Real>;
  std::vector<int> index(size_wout_zero);
  std::iota(index.begin(), index.end(), 0);
  ADoubleSparseWrap as(0,index);
  ADoubleSparseWrap bs(0,index);
  for(Arcane::Integer k = 0; k < nb_test; ++k) {
    ...
    { // 1ere expression sparse
      ...
      as.value() = aa[0] = uniform_generator();
      bs.value() = bb[0] = uniform_generator();
      ...
      as.setGradient(a_grad.unguardedBasePointer());
	  bs.setGradient(b_grad.unguardedBasePointer());
      ...
      ADoubleSparse r_sparse = 3. * (a * b + a);
