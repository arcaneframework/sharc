...
private:
  ...
  unpack(const int size, const audi::scalar<Arcane::Real>& contrib) const
  { return contrib; }

  inline audi::scalar<Arcane::Real>
  unpack(const int size, const audi::sparse_expression<Arcane::Real>& contrib) const
  { return audi::unpack(size, contrib); }
...
_stencil_test()
{
  ...
  auto out1_contrib = Law::contribution<ContributionModel::Out1>(m_domain, m_funcs, 2, um_properties, m_gump_system);
  auto out2_contrib = Law::contribution<ContributionModel::Out2>(m_domain, m_funcs, 2, um_properties, m_gump_system);
  //
  // Input properties used to evaluate reference results
  auto accessor = m_domain.lawVariableAccessor();
  VariableCellReal in_1(accessor.values(Law::cast(in1)));
