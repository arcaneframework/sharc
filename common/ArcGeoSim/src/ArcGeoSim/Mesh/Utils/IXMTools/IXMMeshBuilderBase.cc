...
//! Correspond aux nombres composantes des coordonnees : ici toujours 3 valeurs (x,y,z) meme en 2D
...
staticInit(Arcane::IPrimaryMesh * mesh)
{
  ...
  // Rque PH : On doit pouvoir reduire le nombre de groupe en fonction du contexte (status)
...
//
  // Reduction parallele sur les decisions de modifications collectives
  ...
  // Pretraitements concernant les cellules qui seront supprimees
  // on detache les cellules a supprimer et on supprime des groupes les faces des cellules a supprimer
  ...
  {
      // * 0 - Identification des structures de sous-maillages (cas des cellules supprimees)
      ...
      //* 1- Identification des cellules a supprimer
      ...
      //* 1'- Identification des sous-maillages impactes
      ...
      // on detache les cellules a supprimer

      //* 3- Detachement des cellules a supprimer (sous-maillages inclus)
      // SDC Attention le detachement supprime les connectivites : on ne pourra plus ensuite faire la mise a jour des proprietaires. DEBUG ATTENTION IL FAUT FAIRE LES MISES A JOUR DE OWNER D'ABORD
      // En effet cette derniere necessite de connaitres les sous-items des mailles enlevees...

      //* 3-1 Mise a jour des proprietaires : Il existe en parallele des cas limite ou des sous-items
      // n'ont plus de cellules de meme proprietaire a proximite suite a des suppressions de mailles
      ...
      for(Arcane::Integer i_family = 0; i_family < removed_cell_datas.size(); ++i_family)
      {
          ...
          ENUMERATE_CELL(icell,cell_family->view(removed_cell_data.old_removed_cell_lids))
          {
            ...
            cell_internal->unsetUniqueId(); // permet d'eviter que deux items aient le meme uniqueId meme temporairement
          }

      }


      // on calcule  les faces a supprimer
  ...
  // Traitement des cellules modifiees
  if (global_has_cell_to_change)
    {
      const Arcane::Integer nb_changed_cells = changed_cell_uids.size();

      /* Procedure composee de :
       *            Conservation de l'etat initial des groupes de faces, de noeuds et d'edges
       * 0-         Identification des structures de sous-maillages
       * 1-         Identification des cellules a modifier (topologiquement)
       * 2-         Notification des observers
       * 3-         Detachement des cellules a changer
       *            Permet de ne pas perdre les items associes (faces, nodes...)
       *            Pour aussi permettre une reutilisation de l'uid, l'uid associe est
       *            retire de la table de hachage de la famille apres avoir ete invalide
       *            dans l'ItemInternal
       * 4-         Ajout des nouvelles cellules (qui s'appuyent par defaut sur les
       *            items des cellules precedemment detachees)
       * 4bis-      Creation du tableau des faces ayant de nouveaux id
       * 4ter-      Creation du tableau des noeuds ayant de nouveaux id
       * 4quater-   Creation du tableau des edges ayant de nouveaux id
       ...
       * 6-         Mise a jour de la famille de cellules (et du groupe des cellules modifiees)
       * 6bis-      Mise a jour de la famille de faces
       * 6ter-      Mise a jour de la famille de noeuds
       * 6quater-   Mise a jour de la famille de edges
       * 7-         Copie des valeurs aux mailles aux nouveaux emplacements
       *            (encore associes aux cellules detachees)
       ...
       * 8-         Suppression des cellules detachees
       */
      ...
      { //* 1- Identification des cellules a modifier (topologiquement)
        ...
        // Marquage des proprietaires originaux
      ...
      notifyEvolutiveMeshObservers(IEvolutiveMesh::BeforeUpdateCell);
#endif
      //* 3- Detachement des cellules a changer (sous-maillages inclus)
      ...
      // 4ter Creation du tableau des nodes ayant de nouveaux id
      ...
      // On doit memoriser les modifs avant les endUpdate (avec reordonnement prealable)
      for(Arcane::Integer i_family = 0; i_family < changed_cell_data_size; ++i_family)
        {
          ChangedCellData & changed_cell_data = changed_cell_datas[i_family];
          ...
          std::map<Int64,Int32> uid_to_old_lid; // map pour reordonnement des new_cell_lids
          ...
          for (ItemGroupCollection::Iterator i = coll.begin(); i != coll.end(); ++i)
            {
              ...
              if (!new_cell_del.empty())
                {
                  // group.localIdToIndex() reconstruit systematiquement la table, ce n'est donc pas un critere fiable quant a son existence
                  Int32SharedArray destination_remap, source_remap; // utilise une numerotation en index (et non en localId pour les variables partielles)
                  ...
                  if ( i_family == 0 )
                    {
                      ...
                      // sinon, elle porte deja des valeurs, on ne l'initialise pas
                    }
                }
...
//#endif
                  ...
                  // restriction aux faces nouvellement apparues (resultant de noeuds splittes ou merges avec des noeuds existants)
                  ...
                  if (!old_face_del_reduced.empty())
                    {
                      ...
                      Int32SharedArray source_remap; // utilise une numerotation en index (et non en localId pour les variables partielles)
                      ...
                      for( VariableList::Enumerator i(vars); ++i; ){
                          ...
                          if (variable->isPartial() and variable->itemGroup() == group)
                            {
                              // On passe par un SerializeBuffer pour memoriser les valeurs utiles depuis le IData de la variable
...
//#endif
                ...
                if (!old_node_del_reduced.empty())
                  {
                    // group.localIdToIndex() reconstruit systematiquement la table, ce n'est donc pas un critere fiable quant a son existence
                    Arcane::Int32UniqueArray destination_remap, source_remap; // utilise une numerotation en index (et non en localId pour les variables partielles)
...
//#endif
      ...
      //* 6- Mise a jour de la famille de cellules
      ...
      for(Integer i_family = 0; i_family < changed_cell_data_size; ++i_family)
        {
          ...
          // On n'utilise pas directement IItemFamily::copyItemValues qui ne gere pas correctement les variables en plus
          // de ne pas etre adapte au cas actuel ou les groupes ont deja ete retailles
...
#endif
      // La Mise a jour des proprietaires est maintenant faite au 3- avant le detachement des cellules (une fois les cellules detachees, on perd l'information de leur connectivite...)
  ...
  if (topology_has_changed)
    {
      ...
      { // Les cellules ...
        ...
        // On connait deja le groupe, ce n'est que pour la preparation au endUpdate
      }
      ...
      // Mise a jour finale du maillage
                   ...
                   const bool check)
{
  ...
  // fixme: Bizarre qu'il n'y ait pas besoin de traitement special de forecast_compute ici
  ...
  if (check)
    {
      /* Les controles verifient:
       * - les doublons dans les noeuds mis a jour par coordonnees
       * - les doublons dans les noeuds mis a jour par clone
       * - les doubles mises a jour
       * - les auto-clones (clones d'eux memes)
       * - les nouveaux noeuds jamais mis a jour
       * - la connectivite entre les noeuds mis a jour par clone (ils doivent etre voisins, ici par face)
       */
            ...
            const bool check)
{
  ...
  if (group_mode == "add")
    ...
    {
      // Ecrase toutes autres precedentes modifs sur le groupe et les placent dans le buffer Add
...
_findAllCellChildFamilies(Arcane::IItemFamily * family, Arcane::SharedArray<RemovedCellData> & child_array, IItemFamilySet & scanned_families)
{
  ...
  if (not finder.second) return; // existait deja
...
{
  // Inutile en sequentiel
  if (m_parallel_mng->commSize() == 1) return;

  // On ne s'interesse qu'aux mailles supprimees de removed_cell_group car le reste est suppose deja coherent.
  ...
  // Calcul des groupes d'interet en commencant par le groupe de face de bords de removed_cell_group et interne vis-a-vis de allCells
 ...
 /* EdgeVector edge_group;
  ...
  // tout en excluant l'association a des elements de removed_cell_group
  // Les modifications sont synchronisees par le fait que les items traites sont toujours internes et donc de voisinage completement defini.
  // (le voisinage completement defini vient de la connectivite par noeud du voisinage)

  const Arcane::Integer sid = m_parallel_mng->commRank();

  // Determine les nouveaux proprietaires des noeuds
  {
  ...
  // Determine les nouveaux proprietaires des aretes
  {
  ...
  // Determine les nouveaux proprietaires des faces
  {
          ...
          const Arcane::Int32ConstArrayView cell_connectivity_owners)
{
  ...
  // Si tableau de connectivite, on suit ses consignes de numerotation, sinon libre (ie en sequentiel ou maillage statique eventuellement parallele)
  if (not cell_connectivity_infos.empty())
    {
      ARCANE_ASSERT((not cell_connectivity_owners.empty()),("Incompatible parallel data"));

      // Gestionnaire de types pour mapper les sous-items des cellules a construire
      Arcane::ItemTypeMng* itm = Arcane::ItemTypeMng::singleton();

      // ne gere pas la non-duplication des items crees
      ...
      for(Arcane::Integer icell=0, info_index=0, connectivity_index=0;icell<nb_cells;++icell)
        {
          ...
          // connectivity_index pointe les infos de connectivite issue du mode forecast : uids des aretes et faces
    ...
    {
      if (m_mesh_type == IXMMeshType::StaticMesh) {
        mesh->allocateCells(nb_cells, cell_infos); // static parallel mesh
      ...
      else if (m_mesh_type == IXMMeshType::EvolutiveMesh)// evolutive sequential mesh
        {
          lids.resize(nb_cells);
          mesh->modifier()->addCells(nb_cells, cell_infos, lids);
          if (!mesh->isAllocated())
