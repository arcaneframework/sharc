...
GeometryServiceBase()
{
  m_changed_property = true; // par defaut pour induire directement une erreur si update non appel?
...
getRealVariableProperty(ItemGroup group, IGeometryProperty::eProperty property)
{
  ...
  if (m_changed_property) {
    if (m_policy_tolerance == eLazy)
      traceMng()->info() << "Property requested without validation with update()";
    else
...
_fullUpdate(IMesh * mesh)
{
   ...
   * Si allItems est requis alors les sous-groupes sont delegues (ie reutilisent les calculs de allItems)
   * Le traitement est aussi fait dans l'ordre Face puis Cell pour factoriser les calculs 
   * (ex: centre des faces pour etoilement des volumes)
   */
  ...
  // Traite par maillage la mise a jour des groupes
        ...
        for(PropertyMap::iterator igroup = m_group_property_map.begin(); igroup != m_group_property_map.end(); ++igroup)
          {
            ...
            // Ne traite que les groupes du maillage courant de bon genre (sinon le heritage du calcul n'est pas possible)
            ...
            if (isSameFamily || isSubFamily)
              {
                ...
                for(ItemGroupGeometryProperty::StorageInfos::iterator iprop = group_property.storages.begin();
                    iprop != group_property.storages.end(); ++iprop)
                  {
                    ...
                    // propriete commune avec allItemGroup qui passe la dimension
  ...
  // Mise a jour des autres proprietes (on repasse sur celles deja calculees mais on n'y fait rien)
...
{
  /* L'algo actuel factorise les calculs avec allItems et le maillage est son parent eventuel
   * Si allItems est requis alors les sous-groupes sont delegues (ie reutilisent les calculs de allItems)
   * Le traitement est aussi fait dans l'ordre Face puis Cell pour factoriser les calculs 
   * (ex: centre des faces pour etoilement des volumes)
   */
  ...
  for(Integer ikind=0;ikind<all_kind_size;++ikind) {
    ...
    // 1- On privilegie les calculs par heritage
    if (parent_family)
      {
                ...
                if (isSameFamily)
                  {
                    ...
                    for(ItemGroupGeometryProperty::StorageInfos::iterator iprop = group_property.storages.begin();
                        iprop != group_property.storages.end(); ++iprop)
                      {
                        ...
                        if (property & all_group_property.defined && (isSameKind || IGeometryProperty::isKindIndependent(property)))
                          {
                            // Si besoin de mise a jour et que parent a jour
    ...
    // 2- Le complement via le groupe allItems
    ItemGroup allItemGroup = family->allItems();
    PropertyMap::iterator finder = m_group_property_map.find(allItemGroup.internal());
    if (finder != m_group_property_map.end())
      {
  ...
  // 3- Mise a jour des autres proprietes (on repasse sur celles deja calculees mais on n'y fait rien)
  for(PropertyMap::iterator iprop = m_group_property_map.begin(); iprop != m_group_property_map.end(); ++iprop) 
    {
      if (iprop->first->mesh() == mesh)
        update(iprop->first);
...
//#endif
  ...
  // Suppose que la propriete associe dans allItems existe en tant que variable
