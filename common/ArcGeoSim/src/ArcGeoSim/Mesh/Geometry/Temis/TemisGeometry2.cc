...
TemisGeometry2T<CoordinatesVariable>::
TemisGeometry2T(const CoordinatesVariable & coords)
  ...
  // 2D il faut faire 2*1/4 sur un des nodes 
...
template<typename CoordinatesVariable>
void
TemisGeometry2T<CoordinatesVariable>::ComputeTriangle3::
...
{
  // Hï¿½ritage du cas gï¿½nï¿½ral (pour orientation et mass_center), on ï¿½crase ensuite center avec un nouveau calcul
  typename EuclidianGeometryT<2,CoordinatesVariable>::ComputeTriangle3(this->m_geom).computeOrientedMeasureAndCenter(item, orientation, center, mass_center,coords);

  coords.resize(3);

  if (item.isCell()) { // Cell en 2D
    // Il faut trouver les 2 nodes qui ont la meme coord y  coeff=1/4 sinon coeff=1/2 
    Real un_quart = 1./4. ; 
    Real un_demi = 1./2. ;
    Real EPS_NULL = 1.e-6 ; 

    if (math::abs(this->m_coords[item.node(0)].y - this->m_coords[item.node(1)].y)  < EPS_NULL)
      {
        center = un_quart * this->m_coords[item.node(0)]
          + un_quart * this->m_coords[item.node(1)]
          + un_demi  * this->m_coords[item.node(2)] ; 
        coords[0] = un_quart;
        coords[1] = un_quart;
        coords[2] = un_demi;

        //cout << "01 Cell " << item.uniqueId() << " center " << center ; 
      }
    else if (math::abs(this->m_coords[item.node(2)].y  - this->m_coords[item.node(1)].y )  < EPS_NULL)
      {

        center = un_quart * this->m_coords[item.node(2)]
          + un_quart * this->m_coords[item.node(1)]
          + un_demi  * this->m_coords[item.node(0)] ; 
        coords[0] = un_demi;
        coords[1] = un_quart;
        coords[2] = un_quart;

        //cout << "21 Cell " << item.uniqueId() << " center " << center ; 
      }
    else if (math::abs(this->m_coords[item.node(0)].y  - this->m_coords[item.node(2)].y )  < EPS_NULL)
      {
        center = un_quart * this->m_coords[item.node(2)]
          + un_quart * this->m_coords[item.node(0)]
          + un_demi  * this->m_coords[item.node(1)] ; 
        coords[0] = un_quart;
        coords[1] = un_demi;
        coords[2] = un_quart;

        //cout << "02 Cell " << item.uniqueId() << " center " << center ; 
      }
    else 
      {
...
template<typename CoordinatesVariable>
void
TemisGeometry2T<CoordinatesVariable>::ComputeQuad4::
computeOrientedMeasureAndCenter(const ItemWithNodes & item, Real3 & orientation, Real3 & center, Real3 & mass_center, Array<Real> & coords)
{
  // Hï¿½ritage du cas gï¿½nï¿½ral (pour orientation et mass_center), on ï¿½crase ensuite center avec un nouveau calcul
  typename EuclidianGeometryT<2,CoordinatesVariable>::ComputeQuad4(this->m_geom).computeOrientedMeasureAndCenter(item, orientation, center, mass_center, coords);
...
template<typename CoordinatesVariable>
void
TemisGeometry2T<CoordinatesVariable>::ComputeInvalidSurface::
computeOrientedMeasureAndCenter(const ItemWithNodes & item, Real3 & orientation, Real3 & center, Real3 & mass_center, Array<Real> & coords)
{
  throw FatalErrorException(A_FUNCINFO,"Not supported Item Type");  
}
...
template<typename CoordinatesVariable>
void
TemisGeometry2T<CoordinatesVariable>::ComputeGenericVolume::
computeOrientedMeasureAndCenter(const ItemWithNodes & item, Real & measure, Real3 & center, Array<Real> & coords)
{
  // Choix d'un point arbitraire d'ï¿½toilement, mais "proche" du centre
  Real3 vcenter(0,0,0);
  ...
  // Etoilement des faces puis du volume en tï¿½traï¿½dres
...
template<typename CoordinatesVariable>
void
TemisGeometry2T<CoordinatesVariable>::ComputeInvalidVolume::
computeOrientedMeasureAndCenter(const ItemWithNodes & item, Real & measure, Real3 & center, Array<Real> & coords)
{
  throw FatalErrorException(A_FUNCINFO,"Not supported Item Type");
}
...
template class TemisGeometry2T<VariableNodeReal3>;

template class TemisGeometry2T<SharedMeshVariableScalarRefT<Node,Real3> >;
