...
init()
{
    ...
    m_long_name.setProperty(IUnitsSystemProperty::Volume,"Barrels per day") ;
    ...
    m_long_name.setProperty(IUnitsSystemProperty::GasVolumeFactor,"Barrels per 1E3 cubic feet") ;
    ...
    m_long_name.setProperty(IUnitsSystemProperty::LiqVolFlow,"Barrels per day") ;
    ...
    // g/cm3 -> kg/m3
    m_mult_factors.setProperty(IUnitsSystemProperty::MassDensity,1000.) ;
    m_short_name.setProperty(IUnitsSystemProperty::MassDensity,"g/cm3") ;
    m_long_name.setProperty(IUnitsSystemProperty::MassDensity,"gram per cubic centimeter") ;
    ...
    m_long_name.setProperty(IUnitsSystemProperty::HeatFluxDensity,"British Thermal Unit per day per square feet") ;
...
Real PAUUnitsSystemService::convert(IUnitsSystem::UnitType unit,Real val)
{
  return m_const_factors.getProperty(unit,0.)+val*getMultFactor(unit) ;
}

Real PAUUnitsSystemService::unconvert(IUnitsSystem::UnitType unit,Real val)
{
  return (val-m_const_factors.getProperty(unit,0.))/getMultFactor(unit) ;
}

void PAUUnitsSystemService::convert(IUnitsSystem::UnitType unit,ArrayView<Real> buf)
{
...
void PAUUnitsSystemService::unconvert(IUnitsSystem::UnitType unit,ArrayView<Real> buf)
{
  Real* ptr = buf.unguardedBasePointer() ;
  const Real factor = getMultFactor(unit) ;
  const Real const_factor = getConstFactor(unit) ;
...
ARCANE_REGISTER_SERVICE_PAUUNITSSYSTEM(PAUUnitsSystem,PAUUnitsSystemService);
