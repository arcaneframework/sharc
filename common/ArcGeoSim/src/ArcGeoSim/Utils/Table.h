...
public:
  ...
  Table(String name,Integer size,Real* x,Real* y) 
  {
    m_name = name ;
    m_size = size ;
    if(size<0)
  ...
  Table(String name,Integer size,Real* x,Real* y, bool invx) 
  {
    m_name = name ;
    m_size = size ;
    if(size<0)
...
public:
    ...
    {
        if(m_size==0) return Arcane::FloatInfo<Real>::maxValue() ;
 ...
 private :
    inline Real df(Integer i) { return (m_y[i+1]-m_y[i])/(m_x[i+1]-m_x[i]) ; } 
    inline void evalf(Integer i, Real x, Real* y, Real* dy ) 
    {
        *dy = df(i) ;
        *y =  m_y[i] + *dy * (x-m_x[i]) ; 
    } 
