...
ItemGroupMapAbstract()
  ...
  , m_is_valid(false)
{
  IStackTraceService * stacktrace = platform::getStackTraceService();
  if (stacktrace)
...
_init(const ItemGroup & group) 
{
  ...
  if (not m_stacktrace.empty() and traceMng() != NULL) 
    {
      Trace::Setter traceSetter(traceMng(),"ItemGroupMap");
      traceMng()->debug(Trace::Highest) 
        << "ItemGroupMapAbstract ctor stack trace reminder:\n" 
        << m_stacktrace;
    }
...
stats(std::ostream & o)
{
  ...
  SharedArray<bool> key_done(m_key_buffer.size());
  key_done.fill(false);
