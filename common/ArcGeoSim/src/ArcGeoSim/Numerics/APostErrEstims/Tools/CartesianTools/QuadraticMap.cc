...
void QuadraticMap::compute(const RealSharedArray & aprox_values,
...
void QuadraticMap::__compute_Map(const RealSharedArray & aprox_values,
                                 const RealSharedArray & flux_values,
                                 const VariableDoFReal3x3& perm,
                                 CellsGroup& callCells)
{
  ...
  ENUMERATE_DISCRETIZATION_CELL(icell, callCells) {
   ...
   RealVector fluxLeftSide(numCellFaces);
    ...
    ENUMERATE_FACE(iface,faces){
      ...
      Real coefX1=2*(xF[0]-xT[0])/std::pow(hx,2);
      Real coefY1=2*(xF[1]-xT[1])/std::pow(hy,2);
      Real coefZ1=2*(xF[2]-xT[2])/std::pow(hz,2);
      Real coefX2=1./hx;
      Real coefY2=1./hy;
      Real coefZ2=1./hz;

      quadraticCoefT(i,0)=mF*nTF[0]*(-Perm_T[0][0])*coefX1;
      quadraticCoefT(i,1)=mF*nTF[1]*(-Perm_T[1][1])*coefY1;
      quadraticCoefT(i,2)=mF*nTF[2]*(-Perm_T[2][2])*coefZ1;
      quadraticCoefT(i,3)=mF*nTF[0]*(-Perm_T[0][0])*coefX2;
      quadraticCoefT(i,4)=mF*nTF[1]*(-Perm_T[1][1])*coefY2;
      quadraticCoefT(i,5)=mF*nTF[2]*(-Perm_T[2][2])*coefZ2;
...
*/
    //std::cout<<"solve Quad"<<"\n";
    if(Perm_T[0][0]+Perm_T[1][1]+Perm_T[2][2] !=0){
   ...
   // WARNING TEST == 0
    ...
    lu_substitute(quadraticCoefT, PERM, fluxLeftSide);
     ...
     (*m_quadratic_coef)[cell][6]=aprox_values[cell.localId()] - ((*m_quadratic_coef)[cell][0]+(*m_quadratic_coef)[cell][1]+ (*m_quadratic_coef)[cell][2])/12.;
    }
    else{
...
Real QuadraticMap::eval(const Real3 & P, const Discretization::Cell & cell)
{
	  Measures<Discretization::Cell> cellMeasures(m_dg) ;
  Centers<Discretization::Cell> cellCenters(m_dg) ;
  ...
  Real hx,hy,hz;
  hx = hy = hz = std::pow(cellMeasures(cell), 1./3.);
  const Real3 xT = (cellCenters)(cell) ;// Il faut changer par (cellCenters)(cell) si on veut evaler � un P qui n'est pas le centre de la maille;
  Quadratic Pol(hx,hy,hz,xT,Coef);
  return Pol.eval(P);

}


Real3 QuadraticMap::gradient(const Real3 & P, const Discretization::Cell & cell)
{
	  Measures<Discretization::Cell> cellMeasures(m_dg) ;
  Centers<Discretization::Cell> cellCenters(m_dg) ;
  ConstArrayView<Real> Coef=(*m_quadratic_coef)[cell];
