#ifndef SRC_ERRORESTIMATES_TWODIMENSIONTOOLS_RTNSPACE2D_H_
#define SRC_ERRORESTIMATES_TWODIMENSIONTOOLS_RTNSPACE2D_H_
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
...
public:
  ...
  RtnSpace2D(DiscretizationGeometry* dg ,
  ...
  Real   div(const RealArray & flux_values, const Discretization::Cell & cell);

  typedef Discretization::ItemGroup CellsGroup;
  void compute(const RealArray & flux_values,CellsGroup& callCells);
...
private:
  void __compute_Map(const RealArray & flux_values, CellsGroup& callCells);
  void init();
};
