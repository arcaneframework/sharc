...
{
public:
  Quadratic(const Real& a_hx,         // cell size in x direction
  ...
  inline const Real eval(const Real3 & node) const
  {
    ...
    Real result = m_coef[0]*std::pow((node[0]-m_center[0])/m_hx,2)
                  + m_coef[1]*std::pow((node[1]-m_center[1])/m_hy,2)
                  + m_coef[2]*std::pow((node[2]-m_center[2])/m_hz,2)
                  + m_coef[3]*std::pow((node[0]-m_center[0])/m_hx,1)
                  + m_coef[4]*std::pow((node[1]-m_center[1])/m_hy,1)
                  + m_coef[5]*std::pow((node[2]-m_center[2])/m_hz,1)
                  + m_coef[6];
  ...
  inline const Real3 gradient(const Real3 & node) const
  {
    ...
    result[0]=2*m_coef[0]*std::pow((node[0]-m_center[0])/m_hx,1)/m_hx
      +m_coef[3]/m_hx;

    result[1]=2*m_coef[1]*std::pow((node[1]-m_center[1])/m_hy,1)/m_hy
      +m_coef[4]/m_hy;

    result[2]=2*m_coef[2]*std::pow((node[2]-m_center[2])/m_hz,1)/m_hz
      +m_coef[5]/m_hz;
