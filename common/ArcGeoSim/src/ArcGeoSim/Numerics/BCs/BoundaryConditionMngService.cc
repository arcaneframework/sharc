...
#include "IBoundaryConditionMng.h"

#include "ArcGeoSim/Numerics/Expressions/IFunctionR4vR1.h"
#include "ScalarBoundaryCondition.h"

#include "BoundaryConditionMngService.h"
 ...
 // IAppServiceMng* app_service_mng = IAppServiceMng::instance(subDomain()->serviceMng());
  ...
  for(int i = 0; i < options()->bc.size(); i++) {
    BoundaryConditionPtr bc_ptr =
      new ScalarBoundaryCondition( options()->bc[i]->type(), options()->bc[i]->value() );
  ...
  for(Tag2BoundaryConditionPtrMap::iterator ibc = m_tag_2_bc.begin(); ibc != m_tag_2_bc.end(); ibc++) {
    const ItemGroup& bc_face_group = mesh()->findGroup( ibc->first );
...
FaceGroup BoundaryConditionMngService::getFacesOfType(BoundaryConditionTypes::eType type) {
  ItemGroupBuilder<Face> faces_builder(mesh(), IMPLICIT_NAME);
