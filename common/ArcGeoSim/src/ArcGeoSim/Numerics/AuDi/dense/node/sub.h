#ifndef __audi_dense_node_sub_h__
#define __audi_dense_node_sub_h__

namespace audi {

  template<typename L, typename R>
  struct sub
    : public expression<sub<L,R>>
    , private node
  {
    ...
    sub(const expression<L>& l, const expression<R>& r)
      : m_l(l.staticCast())
      , m_r(r.staticCast())
      , m_lv(m_l.value())
      , m_rv(m_r.value())
    ...
    {
      return m_l.value() - m_r.value();
    }

    inline auto gradient(std::size_t i) const
    {
      return m_l.gradient(i) - m_r.gradient(i);
    }
...
#endif /* __audi_dense_node_sub_h__ */
