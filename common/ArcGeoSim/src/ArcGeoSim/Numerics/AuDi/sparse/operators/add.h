#ifndef __audi_sparse_operators_add_h__
#define __audi_sparse_operators_add_h__
...
inline audi::sparse_expression<T> operator+(const audi::scalar_wrapper<T>& u, const audi::scalar_wrapper<T>& v) {
  return audi::sparse_expression<T>(u.value() + v.value(), audi::make_node(T(1), u, audi::make_node(T(1), v)));
}
...
inline audi::sparse_expression<T> operator+(const audi::scalar_wrapper<T>& u, const audi::sparse_expression<T>& v) {
  return audi::sparse_expression<T>(u.value() + v.first, audi::make_node(T(1), v, audi::make_node(T(1), u)));
}
