...
namespace AlienTools
{
  ...
  template<typename ValueT=Arcane::Real, typename IndexT=Arcane::Integer, int dim_value=3>
  ...
  public :
    ...
    Arcane::UniqueArray<index_type>                           m_dof_is_boundary;
  ...
  public:
    ...
    typedef Alien::IIndexManager::ScalarIndexSet              ScalarIndexSetType;
    typedef Alien::IIndexManager::VectorIndexSet              VectorIndexSetType;
    ...
    public :
      ...
      : m_parent(parent)
      , m_cell(cell)
      , m_matrix(parent.matrixBuffer().unguardedBasePointer(),dim*dof_vector.size(),dim*dof_vector.size())
      , m_rhs(parent.rhsBuffer().unguardedBasePointer(),dim*dof_vector.size())
      ...
      {
        for(int i1=0;i1<m_dof_vector.size();++i1)
        {
          auto const& dof1 = m_dof_vector[i1] ;
          if ( not dof1->isOwn())
            continue;
          ...
          for(int i2=0;i2<m_dof_vector.size();++i2)
          {
            auto const& dof2 = m_dof_vector[i2] ;
    ...
    SystemContributionMng (Alien::Integer maxNumberOfNodesPerCell,
                           Alien::IIndexManager* index_manager,
                           VectorIndexSetType& index_sets,
                           Alien::Array2View<index_type> indexes)
    ...
    Alien::ArrayView<value_type> matrixBuffer() {
      return m_matrix_buffer.view() ;
    }

    Alien::ArrayView<value_type> rhsBuffer() {
  ...
  Alien::Integer                                 m_maxNumberOfNodesPerCell = 0 ;
  Alien::UniqueArray<value_type>                 m_matrix_buffer ;
  Alien::UniqueArray<value_type>                 m_rhs_buffer ;
  IndexManagerType*                              m_index_manager = nullptr;
  VectorIndexSetType                             m_index_sets;
  Alien::Array2View<index_type>                  m_indexes;
