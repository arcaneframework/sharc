...
//! @{
//! \brief Constructeur
  VarVect(int nbParam,double** listeV,int* size) : m_nbParam(nbParam), m_listVect(listeV), m_sizeVect(size)
  {
    ;
  }
//! @}
 ...
 * 
 * La classe Diff est une structure permettant d'evaluer une fonction et ses derivee. Elle est templatisee de sorte de prendre une structure de type VarVect (ou equivalente) en parametre, permettant d'evaluer une fonction. Pour evaluer les derivees de la fonction, elle definit plusieurs autres attributs. \n
 * L'attribut m_varVect est de type VarVect. Se referer a la doc de VarVect pour plus de details. \n
...
//! @{
  ...
  : m_varVect(listeV), m_listDerived(listD), m_derivedValue(derivedV), m_nbVar(nbV)
  {
    ;
  }
//! @}
