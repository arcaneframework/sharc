...
{ \
  ...
  { \
    return pow(left.getValue(index),right.getValue(index)) ; \
  } \
} \

PUISSANCE(Cst<double>)
PUISSANCE(Cst<float>)
PUISSANCE(Arcane::SharedArray<double>)
PUISSANCE(Arcane::SharedArray<float>)
...
{
    return right.getValue(ligne)*left.getDerivedValue(ligne,colonne)* pow(left.getValue(ligne),right.getValue(ligne)-1) ; //+ right.getDerivedValue(ligne,colonne)*log(left.getValue(ligne))*exp(right.getValue(ligne)*log(left.getValue(ligne))) ;
}
...
//! \brief Surcharge de "l'operateur" d'addition pour les types Real3
