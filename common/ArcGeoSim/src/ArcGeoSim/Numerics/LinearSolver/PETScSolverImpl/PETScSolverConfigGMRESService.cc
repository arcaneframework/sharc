...
#include "PETScSolverConfigGMRES_axl.h"
...
class PETScSolverConfigGMRESService :
...
configure(KSP & ksp, const Alien::IIndexManager * indexManager)
{
  ...
  if (options()->useModifiedGramSchmidt())
    checkError("Solver set Modified Gram Schmidt Orthogonalization ",KSPGMRESSetOrthogonalization(ksp,KSPGMRESModifiedGramSchmidtOrthogonalization));
  else
    checkError("Solver set Classical Gram Schmidt Orthogonalization ",KSPGMRESSetOrthogonalization(ksp,KSPGMRESClassicalGramSchmidtOrthogonalization));
