...
#include "PETScSolverConfigSuperLU_axl.h"
...
class PETScSolverConfigSuperLUService :
...
configure(KSP & ksp, const Alien::IIndexManager * indexManager)
{
  ...
  if (parallel_mng->commSize() > 1)
    checkError("Set superlu_dist solver package",PCFactorSetMatSolverPackage(pc,MAT_SOLVER_SUPERLU_DIST));
  else 
    checkError("Set superlu solver package",PCFactorSetMatSolverPackage(pc,MAT_SOLVER_SUPERLU));
#else /* PETSC_VERSION */
  if (parallel_mng->commSize() > 1)
