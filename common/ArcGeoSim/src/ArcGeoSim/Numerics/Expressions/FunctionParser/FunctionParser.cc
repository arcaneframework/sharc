...
#ifndef WIN32
//#warning "BUG: patch with climits to add missing include of boost"
#endif
#endif
...
void 
FunctionParser::
setVariable(const std::string &st, const Real * x, Integer size)
{
  VariableMap::iterator pos = m_var_map.find(st) ;
...
void
FunctionParser::
setVariable(const String &st, const Real * x, Integer size)
{
  setVariable(std::string(st.localstr()),x,size);
}
...
void 
FunctionParser::
setParameter(const String &st, Real x)
{
...
void 
FunctionParser::
setConstant(const String &st, Real x)
{
  std::string s = st.localstr();
  VariableMap::iterator pos = m_var_map.find(s) ;
  if (pos!=m_var_map.end())
...
void 
FunctionParser::
setDerivationResult(const String &di, Real * x, Integer size)
{
      ...
      if(i->second.type == FunctionParserData::PVariable)
        {
          if(i->second.size != m_result_size)
            {
              m_trace_mng->fatal() << "sizes of variables and resut not consistant";
            }
        }
    }
...
//#ifndef NO_USER_WARNING
//#ifndef WIN32
//#warning "TODO PH: optimisation de bas niveau"
//#endif
//#endif
  //#warning "ajouter d�claration variable et constante 'inline'"
//#warning "optimiser avec __restrict__"
//#warning "determiner si le resultat est scalaire � la 'compilation'"
//#warning "connecter les IExpressionMng pour les acc�s aux fonctions; cf apply_func"
...
parseString(const String &s)
{
  ...
  std::string str(s.localstr());
  auto end_pos = std::remove(str.begin(), str.end(), ' ');
  str.erase(end_pos, str.end());
  const parse_info<> info = boost::spirit::classic::parse(str.c_str(), *this >> !end_p, space_p);
