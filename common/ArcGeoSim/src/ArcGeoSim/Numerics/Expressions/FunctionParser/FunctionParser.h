...
//        
// Id�e de perfectionnement
// - Factoriser les buffers d'�valuation par blocking pour l'ensemble des ExpressionBuilder
//   (un pour tous, taill� � la taille max de variable alors)
// - Entr�es de type real3
// - Multiple output (multiple real, real3...)
// - Le traitement des fonctions � plusieurs variables doit etre fait via un traitement du nombre variable d'arguments
//   la sp�cialisation par compte d'arguments pose des probl�mes d'annulation de l'arbre syntaxique
///////////////////////////////////////////////////////////////////
...
struct apply_ref_impl
{
  ...
  void operator()(Container & container, Value value)
  {
    ...
    if (vpos != container.m_var_map.end())
      ...
      { // On cherche la constante nomm�e 'value' dans
        // 1- IExpressionMng local
        // 2- IExpressionMng global
        // 3- Erreur sinon
        // Une fois trouv�e, sa valeur est ajout�e dans la VariableMap
        ...
        if (cpos != container.m_var_map.end())
          ...
          { // vient d'etre ajout�; pourquoi ne pas le trouver
...
struct apply_func_impl
{
  ...
  template <typename Container> void operator()(Container & container, const std::string & func)
  {
    if (func == "cos")
      ...
      container.m_op_stack.push_back(IStackOpPtr(new StackFuncOpT<LogOp>()));
    else if (func == "log10")
      container.m_op_stack.push_back(IStackOpPtr(new StackFuncOpT<Log10Op>()));
    else if (func == "dirac")
      container.m_op_stack.push_back(IStackOpPtr(new StackFuncOpT<DiracOp>()));
//     else if (func == "pow")
...
struct function_closure 
  : public boost::spirit::classic::closure<function_closure, std::string>
{
  member1 name;
};

// Structure permettant de stocker les differents parametres permettant de contruire un element
struct global_closure 
  : public boost::spirit::classic::closure<global_closure, Integer>
{
  member1 dummy;
};
  ...
  public grammar<FunctionParser, global_closure::context_t>, FunctionParserData
{
  ...
  void setVariable(const std::string &st, const Real * x, Integer size);
  void setVariable(const String &st, const Real * x, Integer size);

  //! Fonction pour setter la valeur d'un parametre
  void setParameter(const String &st, Real x);

  //! Fonction pour setter la valeur d'une constante
  void setConstant(const String &st, Real x);

  //! Initialisation du vecteur r�sultat d'�valuation
  void setEvaluationResult(Real * x, Integer size);

  //! Initialisation du vecteur r�sultat d'�valuation
  void setDerivationResult(const String &di, Real * x, Integer size);
