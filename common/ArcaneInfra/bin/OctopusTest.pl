...
my %global_config = ( 'ArcSim' => { 'url'            => 'https://websvn.ifpen.fr/svn/ArcSim',                                                                                                   # svn root url
                                    'compilcmd'      => 'cd $BUILDDIR && cmake -DVerbose=ON -DArcaneVersion=$arcane_version -DARCANE_ROOT=$arcane_path  -DALIEN_ROOT=$alien_path -DDebug=$is_debug .. && make -j $config->{concurrency}', # compil command (will be expanded later)
...
# nettoyage du processus en cas d'arr�t anticip� 
$SIG{INT} = \&cleanup;
...
$alien_path = Cwd::realpath($alien_path) if ($alien_path);
...
if ($config->{verbose}) {
    ...
    print $msg_offset."with Alien=$alien_path\n" if ($alien_path);
...
PROJECT: foreach my $PROJECT (@project_list) {
    ...
    my %is_debug_mode = ( 'debug'   => 'ON', 
                          'release' => 'OFF' );
	
    my $is_debug = $is_debug_mode{$mode};
	
    my $COMPILCOMMAND   = $global_config{$GLOBAL}{compilcmd};
    $COMPILCOMMAND      =~ s/(\$\w+(->{\w+})?)/$1/eeg; # interpolation des variables simples et des r�f�rences de tables de hachage
    $COMPILCOMMAND      =~ s/-DArcaneVersion= //g unless ($arcane_version);
    $COMPILCOMMAND      =~ s/-DARCANE_ROOT= //g unless ($arcane_path);
    $COMPILCOMMAND      =~ s/-DALIEN_ROOT= //g unless ($alien_path);
    $COMPILCOMMAND      =~ s/-DVerbose=ON //g unless ($config->{verbose});
...
Common options: 
      ...
      --alien-path=path       : override selected alien path while testing projects
