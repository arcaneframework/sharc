...
// inspiré de http://stackoverflow.com/questions/860656/using-c-how-does-one-figure-out-what-process-locked-a-file
namespace WindowsZombieKiller
{
    ...
    public class InternalDetectOpenFiles : IDetectOpenFiles
    {
        public InternalDetectOpenFiles() { }
            ...
            foreach (var p in Process.GetProcesses())
            {
                foreach (String fName in files)
                {
                    try
                    {
                        ...
                        {
                            // path match
                            string[] directories = fName.Split(new[] { Path.DirectorySeparatorChar.ToString() }, StringSplitOptions.RemoveEmptyEntries);
                            foreach (String s in InternalDetectOpenFiles.GetOpenFilesEnumerator(p.Id))
                            {
                                String test_path = s;
                                if (File.Exists(test_path))
                                    test_path = Path.GetDirectoryName(s);
                                string[] test_directories = test_path.Split(new[] { Path.DirectorySeparatorChar.ToString() }, StringSplitOptions.RemoveEmptyEntries);
                                if (directories.SequenceEqual(test_directories.Take(directories.Length)))
                                {
                                    Console.WriteLine("Busy resource found on File '{0}' used by process {1}", s, p.Id);
                                    result.Add(p);
                                }
                            }
                        }
                        ...
                        {
                            // file => exact match
                            if (InternalDetectOpenFiles.GetOpenFilesEnumerator(p.Id).Contains(fName))
                            {
                                Console.WriteLine("Busy resource found on File '{0}' used by process {1}", fName, p.Id);
                                result.Add(p);
                            }
                        }
                    }
