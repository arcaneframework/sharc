...
//      { // Enumérateurs d'entités
//        var gen = new Gump.Enumerators ();
//        gen.Session = session;
//        var file = path + "Entities/Enumerators.h";
//        if (verbose) {
//          Console.ForegroundColor = ConsoleColor.Red;
//          Console.WriteLine ("* generate {0}", file);
//          Console.ResetColor ();
...
//      }
      ...
      foreach (var entity in gump.DerivedEntities()) {
        var gen = new DerivedEntity ();
        gen.Session = session;
        session ["entity"] = entity;
        var file = path + "Entities/" + entity.name + ".h";
        if (verbose) {
